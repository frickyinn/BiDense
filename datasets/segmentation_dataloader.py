...
def get_dataLoader(mode, batch_size, num_threads, data_path, base_size, crop_size):
    ...
    data_kwargs = {'transforms': input_transforms, 'base_size': base_size, 'crop_size': crop_size}

    if mode == 'train':
        trainset = ADE20KSegmentation(root=data_path, split=mode, mode=mode, **data_kwargs)
        return DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=num_threads, pin_memory=True)
