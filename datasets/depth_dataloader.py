...
def get_dataLoader(mode, batch_size, num_threads, **kwargs):
    if mode == 'train':
        training_samples = DataLoadPreprocess(mode=mode, transform=preprocessing_transforms(mode), **kwargs)
        return DataLoader(training_samples, batch_size, shuffle=True, num_workers=num_threads, pin_memory=True)

    elif mode == 'online_eval':
        testing_samples = DataLoadPreprocess(mode=mode, transform=preprocessing_transforms(mode), **kwargs)
        return DataLoader(testing_samples, 1, shuffle=False, num_workers=num_threads, pin_memory=True)
    
    elif mode == 'test':
        testing_samples = DataLoadPreprocess(mode=mode, transform=preprocessing_transforms(mode), **kwargs)
        return DataLoader(testing_samples, 1, shuffle=False)
...
class DataLoadPreprocess(Dataset):
    def __init__(
        ...
        self.gt_path_eval = gt_path_eval
