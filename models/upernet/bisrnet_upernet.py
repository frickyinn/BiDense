...
class UperNet(nn.Module):
    ...
    def __init__(self, head, in_channels=3, features=256, backbone='convnext_tiny', pretrained=False):
        ...
        self.out_conv1 = BiSRConv2d(fpn_out, features // 2, kernel_size=3, stride=1, padding=1)
...
class BiSRUperNetDepthModel(UperNet):
    def __init__(self, max_depth, **kwargs):
        ...
        head = nn.Sequential(
            BiSRConv2d(features // 2, 32, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(True),
...
class BiSRUperNetSegmentationModel(UperNet):
    def __init__(self, num_classes, **kwargs):
        ...
        head = nn.Sequential(
            BiSRConv2d(features // 2, features // 2, kernel_size=3, padding=1, bias=False),
