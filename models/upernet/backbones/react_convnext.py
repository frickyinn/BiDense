...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0.):
        super().__init__()
        self.dwconv = ReActConv2d(dim, dim, kernel_size=7, padding=3, groups=dim)  # depthwise conv
        self.norm1 = nn.BatchNorm2d(dim)
        self.pwconv1 = ReActConv2d(dim, 4 * dim, kernel_size=1, stride=1)
        self.norm2 = nn.BatchNorm2d(4 * dim)
        self.move = LearnableBias(4 * dim)
        self.act = nn.PReLU(4 * dim)
        self.pwconv2 = ReActConv2d(4 * dim, dim, kernel_size=1, stride=1)
    ...
    def __init__(self, in_chans=3, num_classes=1000, to_cls=False,
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                ReActConv2d(dims[i], dims[i + 1], kernel_size=2, stride=2),
