...
from binary.react import ReActConv2d, LearnableBias
from .backbones.react_convnext import get_convnext
...
class PSPModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channel=None, bin_sizes=[1, 2, 4, 6]):
        ...
        self.bottleneck = nn.Sequential(
            ReActConv2d(in_channels + (out_channels * len(bin_sizes)), out_channel if out_channel else in_channels, kernel_size=3, padding=1, bias=False),
    ...
    def _make_stages(self, in_channels, out_channels, bin_sz):
        ...
        conv = ReActConv2d(in_channels, out_channels, kernel_size=1, bias=False)
...
class FPN_fuse(nn.Module):
    def __init__(self, feature_channels=[256, 512, 1024, 2048], fpn_out=256):
        ...
        self.conv1x1 = nn.ModuleList([nn.Sequential(
            ReActConv2d(ft_size, fpn_out, kernel_size=1),
            nn.BatchNorm2d(fpn_out),
        ) for ft_size in feature_channels[1:]])
        self.smooth_conv = nn.ModuleList([nn.Sequential(
            ReActConv2d(fpn_out, fpn_out, kernel_size=3, padding=1),
            nn.BatchNorm2d(fpn_out),
        )] * (len(feature_channels) - 1))
        self.conv_fusion = nn.Sequential(
            ReActConv2d(len(feature_channels) * fpn_out, fpn_out, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(fpn_out),
