...
def _make_scratch(in_shape, out_shape, groups=1, expand=False):
    ...
    scratch.layer1_rn = CFBConv2d(in_shape[0], out_shape1, kernel_size=3, stride=1, padding=1, bias=False, groups=groups)
    scratch.layer2_rn = CFBConv2d(in_shape[1], out_shape2, kernel_size=3, stride=1, padding=1, bias=False, groups=groups)
    scratch.layer3_rn = CFBConv2d(in_shape[2], out_shape3, kernel_size=3, stride=1, padding=1, bias=False, groups=groups)
    scratch.layer4_rn = CFBConv2d(in_shape[3], out_shape4, kernel_size=3, stride=1, padding=1, bias=False, groups=groups)
...
class ResidualConvUnit(nn.Module):
    def __init__(self, features):
        ...
        self.conv1 = CFBConv2d(features, features, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv2 = CFBConv2d(features, features, kernel_size=3, stride=1, padding=1, bias=True)
...
class DPT(nn.Module):
    def __init__(self, head, features=256, out_channels=[48, 96, 192, 384], use_clstoken=False):
        ...
        self.projects = nn.ModuleList([
            CFBConv2d(in_channels=in_channels, out_channels=out_channel, kernel_size=1, stride=1, padding=0) 
        ...
        self.resize_layers = nn.ModuleList([
            ...
            nn.Identity(),
            CFBConv2d(in_channels=out_channels[3], out_channels=out_channels[3], kernel_size=3, stride=2, padding=1)
        ])
